...
module.exports = function (roll) {
    ...
    roll.observe("before save", (ctx, next) => __awaiter(this, void 0, void 0, function* () {
        let gameId = ctx.instance.gameId, frameId = ctx.instance.frameId, playerId = ctx.instance.playerId, knockedPins = ctx.instance.knockedPins;
        //if (!ctx.instance) {// create operation}
        let gameModel = roll.dataSource.models.game;
        let game = yield gameModel.findOne({ where: { id: gameId } });
        ...
        if (currentFrames.length == playersCount &&
            currentFrames.every((status) => status === "completed")) {
            game.currentFrame += 1;
        }
